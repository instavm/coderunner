...
CLAUDE_PKG_DIR=${CLAUDE_PKG_DIR:-/usr/local/lib/node_modules/@anthropic-ai/claude-code}
...
# Copy shared libraries needed by node
while IFS= read -r lib; do
  dest="$ROOTFS_DIR${lib}"
  mkdir -p "$(dirname "$dest")"
  cp -L "$lib" "$dest"
done < <(ldd "$NODE_BIN" | awk '{print $3}' | grep '^/')
